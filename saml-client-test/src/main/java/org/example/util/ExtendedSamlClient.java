...
public class ExtendedSamlClient {
  private static final Logger logger = LoggerFactory.getLogger(ExtendedSamlClient.class);
  ...
  public ExtendedSamlClient(
    ...
    this.credentials =
        certificates.stream().map(ExtendedSamlClient::getCredential).collect(Collectors.toList());
  ...
  public void redirectToIdentityProvider(HttpServletResponse response, String relayState)
    ...
    ExtendedBrowserUtils.getUsingBrowser(identityProviderUrl, response, values);
  }
   ...
   * @return The created {@link ExtendedSamlClient}.
   * @throws SamlException thrown if any error occur while loading the metadata information.
   */
  public static ExtendedSamlClient fromMetadata(
    ...
    try {
      certificates =
          idpSsoDescriptor
              ...
              .flatMap(ExtendedSamlClient::getDatasWithCertificates)
              .map(ExtendedSamlClient::getFirstCertificate)
